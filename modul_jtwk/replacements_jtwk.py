...
VOWELS = 'aiuĕāâîīûūêôeèéöoꜽꜷAĀÂIĪÎUŪÛOŌÔEÊÉÈꜼꜶ'
VOWEL_PENDEK = 'aiuĕ'
VOWEL_PANJANG = 'āâîīûūêôoeèéöĀÂÎĪÛŪÊÔÉÈÖŌ'
...
def tandai_vokal_pendek_dalam_pasangan(text):
    ...
    for line in lines:
        ...
        for i in range(len(vokal_matches) - 1):
            ...
            # Menambahkan logika untuk memeriksa urutan vokal panjang dan pendek
            if idx2 - idx1 <= 2:
                if line[idx1+1:idx2].isspace() or idx2 == idx1 + 1:
                    # Cek apakah pasangan vokal panjang dan pendek pada urutan metrum
                    if met1 == '–' and met2 == '⏑':
                        if v1 in VOWEL_PANJANG and v2 in VOWEL_PENDEK:
                            hasil_line[m2.start()] = ZWNJ + v2.upper()
                            skip_next = True
                    elif met1 == '⏑' and met2 == '–':
                        if v1 in VOWEL_PENDEK and v2 in VOWEL_PANJANG:
                            hasil_line[m1.start()] = ZWNJ + v1.upper()
                            skip_next = True
                    elif met1 == '⏑' and met2 == '⏑':
                        # Jika keduanya metrum ⏑, beri ZWNJ dan kapitalisasi vokal kedua
                        hasil_line[m2.start()] = ZWNJ + v2.upper()
                        skip_next = True
