...
vokal_regex = "(" + "|".join(map(re.escape, daftar_vokal)) + ")"
...
def hukum_ṙ(text):
    ...
    #ry (vokal) jadi ry-(vokal)
    text = re.sub(rf'\bry(y)?\b\s+{vokal_regex}',lambda m: f"ry-{m.group(2)}",text)
    ...
    penggantian_spesial = {
        ...
        r'ṙṇny': 'ṙny',
        r'aṙyy(a|ā)': r'ary\1',
        r'āś(c|ꞓ)ary' : r'āś\1aṙyy',
        r'ṙyyakĕn\b': 'ryakĕn',
        r'uṙww': 'urw',
        r'ṙmmu ': 'ṙmu ',
        r'[^\S\n]+lĕ': ' ‌lĕ',     #zwnj lĕ untuk mencegah lĕ dibawah untuk kakawin
...
def finalisasi(text):
    ...
    text = re.sub(rf'([^\w\s-])(\s*)([{daftar_vokal}])',lambda m: f"{m.group(1)}{m.group(2)}{m.group(3).upper()}",text)
